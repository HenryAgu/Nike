  ...
  return (
    <section className="py-20 lg:py-26 px-10 lg:px-24 xl:px-40">
      <div className="flex items-start justify-center">
        <Image src={RaysIcon} alt="rays" className="relative bottom-3 left-0" />
        <h3
          className={`${TacticSansBold.className} text-lg md:text-xl lg:text-[32px] xl:text-[40px] font-normal text-white text-center uppercase text-stroke`}
        >
      ...
      <div className="bg-[#FF00000D] my-8 lg:my-16 h-fit lg:pr-20 rounded-2xl lg:rounded-tl-[231px] lg:rounded-bl-[231px] lg:rounded-tr-[98px] lg:rounded-br-[98px] flex lg:flex-row flex-col justify-between items-stretch">
        ...
        <div className="flex flex-col items-center lg:items-end text-center lg:text-right gap-y-5  py-12">
          <p
            className={`${TacticSansBold.className} font-normal text-2xl md:text-3xl lg:text-4xl xl:text-5xl text-black lg:leading-12 xl:leading-14`}
          >
            AIR JORDAN <br /> Retro 7
          </p>
          <div className="flex items-center gap-x-3.5">
            <span className="bg-brand-gray-250 text-brand-gray-300 px-2 text-base md:text-lg lg:text-xl xl:text-[22px] font-normal">
              Men’s Shoes
            </span>
            <span className="text-base md:text-lg lg:text-xl xl:text-[22px] text-black font-normal">2 Colors</span>
          </div>
          <p
            className={`${TacticSansBold.className} text-[#4B4B4B] text-2xl md:text-3xl lg:text-[34px] xl:text-[38px] font-normal`}
          >
            MRP: $229.99
          </p>
          <button
            type="button"
            className="bg-[#FE0101] text-white py-2 px-3.5 text-sm md:text-base lg:text-lg font-normal cursor-pointer transition-all duration-200 ease-in-out hover:opacity-65"
          >
            Add to bag +
          </button>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 md:gap-5 gap-10 px-10 md:px-0 my-10 md:my-0">
